...
public class RentedItemsActivity extends AppCompatActivity {
   ...
   MyAdapter adapter_;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("RENTED ITEMS");

        RecyclerView recyclerView=(RecyclerView) findViewById(R.id.recyclerView);
        recyclerView.setLayoutManager(new LinearLayoutManager(getApplicationContext()));

        FirebaseAuth mAuth = FirebaseAuth.getInstance();
        String renter = mAuth.getUid();
        FirebaseRecyclerOptions<Item> options =
                new FirebaseRecyclerOptions.Builder<Item>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("Items").orderByChild("renter").startAt(renter).endAt(renter+"\uf8ff"), Item.class)
                        .build();

        adapter_ = new RentedItemsActivity.MyAdapter(options);
        ...
        public void onBindViewHolder(@NonNull com.example.lor277project.MyAdapter.myviewholder holder, int position, @NonNull Item item)
        {
            ...
            holder.v.setOnClickListener(new View.OnClickListener(){
                ...
                public void onClick(View v){
                    Intent intent = new Intent(RentedItemsActivity.this, ViewRentedItemsActivity.class);
